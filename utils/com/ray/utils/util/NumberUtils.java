...
final public class NumberUtils {
	...
	public static long randomLong(Random rand, long min, long max) {
		return (long)random(min, max);
	}
	 ...
	 * 求m和n的最大公约数   
	 */
    public static int commonDivisorMax(int m, int n) {  
    	int min = m, max = n;
    	if(m > n){
    		min = n;
    		max = m;
     ...
     * 求m和n的最小公倍数
     */
    public static int commonMultipleMin(int m, int n) {   
        return m * n / commonDivisorMax(m, n);   
    }
